...
const { APP_PORT, APP_HOST, PRODUCT_PATH } = require('@config/const')


// 创建商品
const createProduct = async (ctx, next) => {
    // 商品名称，价格，原价，商品描述，生产地址，库存，库存单位，上架状态
    const { product_name, price, old_price = 0, description = '', product_address = '', stock, stock_unit, publice_status } = ctx.req.body
    // // 商品预览图
    const { filename } = ctx.req.file

    const sql = "INSERT INTO products (id,userId, product_name, price, old_price, description, picture, product_address, stock, stock_unit, publice_status) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
...
const getAllProduct = async ctx => {
    ...
    const sql = `SELECT * FROM products ORDER BY price ${asc == 0 ? "ASC" : "DESC"} LIMIT ?, ?`
