...
const createMenu = async ctx => {
    const { menu_name, price, description, material, taste, makein, discount } = ctx.req.body
    const { filename, mimetype, size } = ctx.req.file
    const menueID = snid.generate()
    const pictureURL = `${APP_HOST}/menu/${filename}`


    const sql = "INSERT INTO menus (id, menu_name, price, description, picture, material, taste, makein, discount) VALUES (?,?,?,?,?,?,?,?,?)"

    await query(sql, [menueID, menu_name, price, description, pictureURL, material, taste, makein, discount]).then(res => {
        ctx.body = {
            status: 200,
            message: "创建成功"
        }
        ...
        await query(find, [id]).then(async (res) => {
            if (res.length <= 0) {
                ...
                if (picture) {
                    ...
                    if (fileName) {
                        const deletePath = path.join(__dirname, "../../../../img/menu", fileName)
                        await deletFile(deletePath)
                    }
                }
                const del = "DELETE FROM menus WHERE id =?"
                await query(del, [id]).then(res => {
                    ctx.body = {
                        status: 200,
                        message: "删除成功"
                    }
...
const getMenuList = async ctx => {
    ...
    const sql = `SELECT * FROM menus ORDER BY price ${asc == 0 ? "ASC" : "DESC"} LIMIT ?, ?`
